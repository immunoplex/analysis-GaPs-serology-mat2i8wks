...
# ============================================================
# WT_IgG Subclass Network Analysis — Stratified by Infant Arm
# Parallel to WT_IgG_ratio_models chunk in
# response_matpm_whole_model_v1.Rmd
# ============================================================
...
# ---- 1. Feature selection (same as original) ---------------
...
communities <- c(1, rep(2,4), rep(3,4), rep(4,4))   # WHOLE=1, PT=2, FHA=3, PRN=4
...
for (iarm in infant_arms) {
  ...
  data0259_iarm <- data[data$feature %in% wt_igg_features &
  ...
  cat("--- Ratio summary ---\n")
  print(summary(data_wide_iarm[, c("PT_ratio","FHA_ratio","PRN_ratio","WHOLE_WT_IgG")]))
  ...
  cat("\n--- Antigen-level model (total IgG → WT_IgG) ---\n")
  model_antigen_i <- lm(WHOLE_WT_IgG ~ PT_IgG + FHA_IgG + PRN_IgG,
                         data = data_wide_iarm)
  ...
  cat("\n--- PT subclass model ---\n")
  model_PT_i  <- lm(WHOLE_WT_IgG ~ PT_IgG1  + PT_IgG2  + PT_IgG3  + PT_IgG4,
                     data = data_wide_iarm)
  print(calc.relimp(model_PT_i, type = "lmg"))

  cat("\n--- FHA subclass model ---\n")
  model_FHA_i <- lm(WHOLE_WT_IgG ~ FHA_IgG1 + FHA_IgG2 + FHA_IgG3 + FHA_IgG4,
                     data = data_wide_iarm)
  print(calc.relimp(model_FHA_i, type = "lmg"))

  cat("\n--- PRN subclass model ---\n")
  model_PRN_i <- lm(WHOLE_WT_IgG ~ PRN_IgG1 + PRN_IgG2 + PRN_IgG3 + PRN_IgG4,
                     data = data_wide_iarm)
  print(calc.relimp(model_PRN_i, type = "lmg"))

  # ---- 2e. Full 12-subclass model -------------------------

  cat("\n--- Full 12-subclass model ---\n")
  model_full_i <- lm(
    WHOLE_WT_IgG ~ PT_IgG1  + PT_IgG2  + PT_IgG3  + PT_IgG4  +
                   FHA_IgG1 + FHA_IgG2 + FHA_IgG3 + FHA_IgG4 +
                   PRN_IgG1 + PRN_IgG2 + PRN_IgG3 + PRN_IgG4,
    data = data_wide_iarm)
  ...
  cat("\n--- Ratio model (all timepoints combined) ---\n")
  model_ratio_i <- lm(WHOLE_WT_IgG ~ PT_ratio + FHA_ratio + PRN_ratio,
                       data = data_wide_iarm)
  ...
  cat("\n--- Antigen × Subclass × Value interaction (lmer) ---\n")
  model_int_i <- lmer(
    WHOLE_WT_IgG ~ antigen * subclass * subclass_value + (1 | subject_accession),
  ...
  if (nrow(data_net_i) >= 50) {
    ...
    plot(network_i,
         ...
         labels      = node_names,
         color       = group_colors,
         title       = paste0("Visit 5 - ", iarm, " infants"),
         legend      = TRUE,
         legend.cex  = 0.4,
         vsize       = 8,
         border.width = 1.5,
    ...
    corrplot(adj_i, method = "color", type = "lower",
             ...
             title  = paste0("Partial Correlation - Visit 5 - ", iarm),
...
cat("\n\n============================================================\n")
cat("  CROSS-ARM: aP vs wP network comparison at Visit 5\n")
cat("============================================================\n\n")
...
cat("\n--- Bridge centrality comparison: aP vs wP ---\n")
...
cat("\n--- Relative importance comparison: aP vs wP ---\n")
...
ggplot(ratio_coef, aes(x = term, y = estimate, fill = infant_arm,
  ...
  labs(x = "Pro-inflammatory/Tolerogenic Ratio",
       y = "Regression Coefficient (→ WT_IgG)",
       title = "Ratio model: aP vs wP infants",
       fill  = "Infant arm") +
  theme_bw()
