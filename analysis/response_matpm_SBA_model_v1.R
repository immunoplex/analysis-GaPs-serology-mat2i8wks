# ---
# title: "SBA associations with IgG total and IgG subclasses binding to pertussis antigens"
# subtitle: "Stratified by infant arm (aP vs wP) — parallel to WT_IgG analysis"
# author: "Scot Zens"
# ---
...
# ============================================================
# SBA Network Analysis — Stratified by Infant Arm
# Parallel to WT_IgG_ratio_models chunk in
# response_matpm_whole_model_v2.R — outcome switched to
# WHOLE_SBA; all predictors are pertussis-antigen IgG total
# and IgG subclasses (PT, FHA, PRN × IgG1–4)
# ============================================================
...
# Network nodes: SBA + 12 pertussis-antigen subclasses
net_vars <- c("WHOLE_SBA",
...
node_names <- net_vars
groups_idx <- list(
  "SBA" = which(node_names == "WHOLE_SBA"),
...
communities <- c(1, rep(2,4), rep(3,4), rep(4,4))   # SBA=1, PT=2, FHA=3, PRN=4
...
for (iarm in infant_arms) {
  ...
  data0259_iarm <- data[data$feature %in% sba_features &
  ...
  cat("--- Ratio summary ---\n")
  print(summary(data_wide_iarm[, c("PT_ratio","FHA_ratio","PRN_ratio","WHOLE_SBA")]))
  ...
  cat("\n--- Antigen-level model (total IgG → SBA) ---\n")
  model_antigen_i <- lm(WHOLE_SBA ~ PT_IgG + FHA_IgG + PRN_IgG,
                         data = data_wide_iarm)
  ...
  cat("\n--- PT subclass model (→ SBA) ---\n")
  model_PT_i <- lm(WHOLE_SBA ~ PT_IgG1  + PT_IgG2  + PT_IgG3  + PT_IgG4,
                    data = data_wide_iarm)
  print(calc.relimp(model_PT_i, type = "lmg"))

  cat("\n--- FHA subclass model (→ SBA) ---\n")
  model_FHA_i <- lm(WHOLE_SBA ~ FHA_IgG1 + FHA_IgG2 + FHA_IgG3 + FHA_IgG4,
                     data = data_wide_iarm)
  print(calc.relimp(model_FHA_i, type = "lmg"))

  cat("\n--- PRN subclass model (→ SBA) ---\n")
  model_PRN_i <- lm(WHOLE_SBA ~ PRN_IgG1 + PRN_IgG2 + PRN_IgG3 + PRN_IgG4,
                     data = data_wide_iarm)
  print(calc.relimp(model_PRN_i, type = "lmg"))

  # ---- 2e. Full 12-subclass model -------------------------

  cat("\n--- Full 12-subclass model (→ SBA) ---\n")
  model_full_i <- lm(
    WHOLE_SBA ~ PT_IgG1  + PT_IgG2  + PT_IgG3  + PT_IgG4  +
                FHA_IgG1 + FHA_IgG2 + FHA_IgG3 + FHA_IgG4 +
                PRN_IgG1 + PRN_IgG2 + PRN_IgG3 + PRN_IgG4,
    data = data_wide_iarm)
  ...
  cat("\n--- Ratio model (all timepoints combined, → SBA) ---\n")
  model_ratio_i <- lm(WHOLE_SBA ~ PT_ratio + FHA_ratio + PRN_ratio,
                       data = data_wide_iarm)
  ...
  ratio_by_visit <- data_wide_iarm %>%
    ...
    do(tidy(lm(WHOLE_SBA ~ PT_ratio + FHA_ratio + PRN_ratio, data = .))) %>%
  ...
  cat("\n--- Antigen × Subclass × Value interaction (lmer, → SBA) ---\n")
  model_int_i <- lmer(
    WHOLE_SBA ~ antigen * subclass * subclass_value + (1 | subject_accession),
  ...
  if (nrow(data_net_i) >= 50) {
    ...
    plot(network_i,
         ...
         labels       = node_names,
         color        = group_colors,
         title        = paste0("Visit 5 — ", iarm, " infants (SBA network)"),
         legend       = TRUE,
         legend.cex   = 0.4,
         vsize        = 8,
         border.width = 1.5,
    ...
    corrplot(adj_i, method = "color", type = "lower",
             ...
             title  = paste0("Partial Correlation — Visit 5 — ", iarm, " (SBA network)"),
...
cat("\n\n============================================================\n")
cat("  CROSS-ARM: aP vs wP network comparison at Visit 5 (SBA)\n")
cat("============================================================\n\n")
...
cat("\n--- Bridge centrality comparison: aP vs wP (SBA network) ---\n")
...
cat("\n--- Relative importance comparison: aP vs wP (SBA) ---\n")
...
ggplot(ratio_coef, aes(x = term, y = estimate, fill = infant_arm,
  ...
  labs(x     = "Pro-inflammatory/Tolerogenic Ratio",
       y     = "Regression Coefficient (→ SBA)",
       title = "Ratio model: aP vs wP infants (outcome = SBA)",
       fill  = "Infant arm") +
  theme_bw()
...
ggplot(data_long_combined,
       aes(x = subclass_value, y = WHOLE_SBA,
           color = subclass, linetype = infant_arm)) +
  geom_smooth(method = "lm", se = TRUE, linewidth = 0.8) +
  facet_grid(antigen ~ infant_arm) +
  scale_color_brewer(palette = "Set1") +
  labs(x        = "Log10 Subclass-Specific IgG",
       y        = "Log10 SBA (Serum Bactericidal Activity)",
       color    = "Subclass",
       linetype = "Infant arm",
       title    = "Antigen × Subclass → SBA: aP vs wP infants") +
